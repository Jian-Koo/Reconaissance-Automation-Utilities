...
def startup_header():
    ...
    print("[!] Note         : False positives may occur")
    print("------------------------------------------------------\n")
...
def is_wordpress(domain: str, timeout: int) -> bool:
    ...
    adapter = HTTPAdapter(max_retries=0)  # IMPORTANT
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=args.workers) as executor:
        ...
        try:
            for future in as_completed(futures, timeout=total * args.timeout):
                sub = futures[future]
                completed += 1
            ...
            print("\n[!] Timeout reached — skipping unresponsive hosts")
